import {
  ...
  makeCalculatedTableTsv
} from "../../core/table-column-calculator.js";
...
export async function runTableColumnCalculator(input, options = {}, context = {}) {
  context.reportProgress?.({ phase: "parsing-input", progress: 0.05 });
  context.throwIfCancelled?.();
  await context.yieldIfNeeded?.();

  const result = calculateTableColumn(input, options);
  ...
  const tsv = makeCalculatedTableTsv(result.outputColumns, result.outputRows);
  const outputs = {
    tsv,
  ...
  return makeToolResult({
    ...
    download: {
      filename: outputFormat === "report" ? "table-column-calculator.txt" : "table-column-calculator.tsv",
    ...
    streams: {
      ...
      table: makeTableStream(result.outputColumns, result.outputRows, "table-column-calculator")
    }
  });
}
