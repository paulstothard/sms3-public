...
export async function runLinePlot(input, options = {}, context = {}) {
  context.reportProgress?.({ phase: "plotting-table", progress: 0.1 });
  context.throwIfCancelled?.();
  await context.yieldIfNeeded?.();

  const result = makeLinePlot(input, options);
  const outputFormat = options.outputFormat === "point-tsv" ? "point-tsv" : "svg";
  const tsv = plotRowsToTsv(linePlotColumns, result.rows);
  ...
  return makeToolResult({
    output,
    download: {
      filename: outputFormat === "point-tsv" ? "line-points.tsv" : "line-plot.svg",
    ...
    streams: {
      pointTable: makeTableStream(linePlotColumns, result.rows, "line-points")
    },
